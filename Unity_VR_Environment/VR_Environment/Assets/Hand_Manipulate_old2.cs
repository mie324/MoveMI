...
public class Hand_Manipulate_old2 : MonoBehaviour {
    ...
    {
        {
            {254f, 237.6f, 238.6f, 238.6f}, {258.5f, 275.5f, 238.6f, 238.6f}, {245.6f, 226.2f, 194.7f, 238.6f}
        },
        {
            {345f, 323.5f, 316.7f, 316.7f}, {3.7f, 350.2f, 330.7f, 316.7f}, {269.4f, 270.3f, 252.7f, 316.7f}
        },
        {
            {351.1f, 323.9f, 316.3f, 316.3f}, {1.2f, 346.1f, 327.4f, 316.3f}, {289.7f, 279.4f, 235.2f, 316.3f}
        },
        {
            {350.1f, 326.7f, 317.1f, 317.1f}, {2.8f, 340.5f, 329.1f, 317.1f}, {297.9f, 270.6f, 246.5f, 317.1f}
        },
        {
            {179.9f, 345.5f, 326.6f, 321.2f}, {0.9f, 351.6f, 341.6f, 333.3f}, {166.1f, 317.6f, 239.1f, 274.9f}
        }
    };
    ...
    void fingerMove(int finger, int action)
    {
        ...
        } else
        {
            if (Time.time - start_time <= 1)
            {
                ...
                {
			if(action == 1) {
			    if(finger < 4) curr_finger.transform.Rotate(0, 0, angles[i] * 0.8f * Time.deltaTime);
			    else curr_finger.transform.Rotate(0, 0, angles[i] * 0.5f * Time.deltaTime);
			} else if(action == 2) {
			    if(finger < 4) curr_finger.transform.Rotate(0, 0, angles[i] * 2f * Time.deltaTime);
			    else curr_finger.transform.Rotate(0, 0, angles[i] * 2f * Time.deltaTime);
			}
			if (i < 3) curr_finger = curr_finger.transform.GetChild(0).gameObject;
                }
            }
